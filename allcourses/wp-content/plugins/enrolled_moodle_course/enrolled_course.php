...
$datav=$wpdb->get_row("select * from " . $wpdb->prefix . "wootomoodle where productid='".$product->ID."'");
if(empty($datav)){
                        ?>
                    ...
                    <div class="form-group">
                        ...
                        <select name="course" class="form-control">
                            ...
                            while ($row = $result->fetch_assoc()) {
                                // echo "id: " . $row["id"]. "<br>";
                                $datav=$wpdb->get_row("select * from " . $wpdb->prefix . "wootomoodle where courseid='".$row['id']."'");
if(empty($datav)){
                                ?>
...
<div class="row">
    ...
    <?php echo date('d/m/Y h:i:s', strtotime($myrowsdata->datecreated)); ?>
                        ...
                        <td><a class="btn btn-danger" 
                            onclick="wootomoodledelete(<?php echo $myrowsdata->id; ?>,<?php echo $myrowsdata->productid;?>,<?php echo $myrowsdata->courseid; ?>)">Delete</a></td>
                    </tr>
                    <?php
                    $i++;
                }
...
<script>
            ...
            success: function (data) {
                ...
                if (data == 1) {
                    $('#error').html('Data already exist');
                                    ...
                                    jQuery.ajax({
                        type: "POST",
                        url:'<?php echo $mdllists->moodleurl; ?>/blocks/customhomepage/jquery.php',
                        data: {
                            action: 'insert_moodle_data',
                            md_productID: md_productID,
                            mdl_courseID: mdl_courseID
                        },
                        success: function (data) {
    ...
    function wootomoodledelete(id,md_productID,mdl_courseID)
    {
         // alert(id+"xxx"+md_productID+"cccc"+mdl_courseID
         //    );
