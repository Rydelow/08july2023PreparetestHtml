...
class Allseodata 
{
    ...
    public static function seodashboardthirdcategorystatus(){
    	...
    	if(!empty($_GET['status'])){
    		$data=$DB->get_record("searchda_categories_seo",array('id'=>$_GET['status']));
    		...
    		redirect(new moodle_url("/local/seodashboard/allseodata.php?id=".$_GET['id']), 'Sucessfully data updated', null, \core\output\notification::NOTIFY_SUCCESS);
    	...
    	if(!empty($_GET['delete'])){
			 	 ...
			 	 $sqlquerydataa = "DELETE FROM {searchda_categories_seo} where id='".$_GET['delete']."'";
		         $DB->execute($sqlquerydataa);

    		redirect(new moodle_url("/local/seodashboard/allseodata.php?id=".$_GET['id']), 'Sucessfully data Deleted', null, \core\output\notification::NOTIFY_SUCCESS);
    ...
    public function index(){
        ...
        if(!empty($_GET['cid'])){
        	...
        	$thirdcategory=$this->extra->thirdcategory_option($cid,$thid);
        }
        ...
        $setting=array('menurecord'=>$this->recorddispay(),'currenturlid'=>$currenturlid,'thirdcategory'=>$thirdcategory,'filterdata'=>self::searchda_categories_seo(),'addbutton'=>$addbtton,"style"=>$css,'title'=>$title,'category'=>$this->extra->firstcategory_option($id),'subcategory'=>$this->extra->secondcategory_option($id,$cid));
       return $OUTPUT->render_from_template('local_seodashboard/default', $setting);
    }
    public function menu_nav(){
    	 ...
    	 if(!empty($_GET['thid'])){
    	 	$rdata=$DB->get_record('searchda_third',array('id'=>$_GET['thid'],'status'=>'0'));
    	 	$ldata['menu3']=$rdata->title;
    	 }
    	 return $ldata;
