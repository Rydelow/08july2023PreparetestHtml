...
//     }
    public function index(){
       ...
       if(!empty($_GET["errorcode"])){
          if ($_GET["errorcode"] == 3){
             $errorcode='<div class="error_msg">The Username or Password entered is invalid.</div>';
          }
          if ($_GET["errorcode"] == 2){
             $errorcode='<div class="error_msg">The Username  entered is invalid.</div>';
          }

          if ($_GET["errorcode"] == 4){
             $errorcode='<div class="error_msg">Session Expired, please try again.</div>';
          }
       }
    	$setting=array('root'=>$CFG->wwwroot,'sesskey'=>sesskey(),'postdata'=>$sdata,'errorcode'=>$errorcode);
