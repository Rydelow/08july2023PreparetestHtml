...
if (is_null($ADMIN->locate('seodashboard'))) {
    $ADMIN->add( 'roott', new admin_category( 'seodashboard', get_string('pluginname', 'local_seodashboard')));
}

if (is_null($ADMIN->locate('IomadReports'))) {
    $ADMIN->add('seodashboard', new admin_category( 'seodashboard',
              get_string('pluginname', 'local_seodashboard')));
}
