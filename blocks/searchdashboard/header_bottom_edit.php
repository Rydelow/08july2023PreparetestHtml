...
include('form_header_bottom.php');
...
if(!empty($_GET['id'])){
$PAGE->navbar->add(($topbardata->title), new moodle_url('/blocks/searchdashboard/header_bottom_edit.php?id='.$id));
$PAGE->set_url('/blocks/searchdashboard/header_bottom_edit.php?id='.$id);
}
  ...
  ?>
    ...
    <div class="row">
        <div class="col-md-10">
            <div class="he_ader ">
               <div class="edit1 ">
              <?php echo $topbardata->title; ?> Title Section
               </div>
...
$topbardataa = $DB->get_record('searchda_headerbottom',array('thid'=>$id));
...
if ($mform->is_cancelled()) {
 ...
 $topbar_slider->title=$fromform->title;
  ...
  $DB->insert_record('searchda_headerbottom', $topbar_slider,true);
redirect($CFG->wwwroot."/blocks/searchdashboard/exam_explore.php?sucessfully=inserted&id=".$fromform->thid);
  ...
  $DB->update_record('searchda_headerbottom', $topbar_slider,true);
