  ...
  ?>
    ...
    <div class="row">
        <div class="col-md-10">
            <div class="he_ader ">
               <div class="edit1 ">
              <?php echo $topbardata->title; ?> Add Quiz Section
               </div>
...
$topbardataa = $DB->get_record('searchda_quiz',array('id'=>$cid));
$citydata=$DB->get_records_sql("SELECT * FROM {exam_listing} where `exam_id`='".$cid."' and `exam_table`='searchda_quiz'");
$array=array();
foreach ($citydata as $value) {
  $exam_categoryid=$value->exam_categoryid;
  if($value->action_country=='India'){
    array_push($array,'India');
  }else{
    if(!empty($value->action_country)){
      if($value->action_country=='state'){
        $actionid="state-".$value->state_id;
      }
       if($value->action_country=='city'){
        $actionid="city-".$value->city_id;
       }
      if($value->action_country=='district'){
        $actionid="district-".$value->district_id;
        }
      array_push($array, $actionid);
    }else{
      //empty
    }
  }
...
$setdata->offer=$topbardataa->offer;
$setdata->createdtime=time();
$setdata->areaids=$array;
$setdata->oldexamcity=json_encode($array);
$setdata->exam_category=$exam_categoryid;
...
$topbar_slider->createdtime=time();
  ...
  $insertid=$DB->insert_record('searchda_quiz', $topbar_slider,true);
//   echo"<pre>";
// print_r($insertid);
// die();
...
$('#updateData').click(function () {
 ...
 var oldexamcity=$('input[name=oldexamcity]').val();
   var exam_category=$('#id_exam_category').val();
 var examcitydata = $('#fitem_id_areaids .form-autocomplete-selection span').map(function() {
    return $(this).data('value');
}).get();
console.log(JSON.stringify(examcitydata));
    ...
    success: function(data)
        {
         ...
         var postid=data;
			if (postid) {
						    ...
						    data: { 'action':'updateproduct','oldexamcity':oldexamcity,'exam_category':exam_category,'examcitydata':JSON.stringify(examcitydata),'title':title,'duedate': date,'question':question,'qmark':qmark,'mark':mark,'qumark':qumark,'minute':minute,'qminute':qminute,'price':price,'postid':postid,'moodleurl':moodleurl,'thid':thid,'id':id,'offer':offer,'quiz_type':quiz_type},
						    cache: false,
    ...
    success: function(data)
        {
         ...
         var postid=data;
			if (postid) {
						    ...
						    type: "POST",
						    url: "<?php echo $CFG->wwwroot; ?>/blocks/searchdashboard/ajaxdata.php",
						    data: { 'action':'createproduct','exam_category':exam_category,'examcitydata':JSON.stringify(examcitydata),'title':title,'duedate': date,'question':question,'qmark':qmark,'mark':mark,'qumark':qumark,'minute':minute,'qminute':qminute,'price':price,'postid':postid,'moodleurl':moodleurl,'thid':thid,'offer':offer,'quiz_type':quiz_type },
						    cache: false,
						    success: function(ndata)
						        {
						  $("form").trigger('reset');      	
