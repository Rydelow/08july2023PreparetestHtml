...
include('form_categories_quiz.php');
...
$topbardataa = $DB->get_record('searchda_ca_quiz',array('id'=>$cid));
//echo $topbardata->slider_content['text'];
// $mform->set_data($topbardata);
$citydata=$DB->get_records_sql("SELECT * FROM {exam_listing} where `exam_id`='".$cid."' and `exam_table`='searchda_ca_quiz'");
$array=array();
    ...
    success: function(data)
        {
         ...
         var postid=data;
			if (postid) {
						    ...
						    data: { 'action':'categoriesupdateproduct','oldexamcity':oldexamcity,'exam_category':exam_category,'examcitydata':JSON.stringify(examcitydata),'title':title,'duedate': date,'question':question,'qmark':qmark,'mark':mark,'qumark':qumark,'minute':minute,'qminute':qminute,'price':price,'postid':postid,'moodleurl':moodleurl,'thid':thid,'id':id,'quiz_type':quiz_type,'quizcategories':quizcategories },
						    cache: false,
    ...
    success: function(data)
        {
         ...
         var postid=data;
			if (postid) {
						    ...
						    type: "POST",
						    url: "<?php echo $CFG->wwwroot; ?>/blocks/searchdashboard/ajaxdata.php",
						    data: { 'action':'categoriescreateproduct','exam_category':exam_category,'examcitydata':JSON.stringify(examcitydata),'title':title,'duedate': date,'question':question,'qmark':qmark,'mark':mark,'qumark':qumark,'minute':minute,'qminute':qminute,'price':price,'postid':postid,'moodleurl':moodleurl,'thid':thid,'quiz_type':quiz_type,'quizcategories':quizcategories },
						    cache: false,
						    success: function(ndata)
						        {
						//   $("form").trigger('reset');      	
