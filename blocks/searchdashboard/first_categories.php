...
<div class="container">
    <div class="row">
        <div class="col-md-10">
         ...
         <div class="edit pull-right"> <a href="<?php echo $CFG->wwwroot ?>/blocks/searchdashboard/first_categories.php">View</a> |
       <!--- <a href="#">Edit</a> |--->
...
<div class="row">
  <div class="col-md-10">
    <table id="example" class="display" cellspacing="0" width="100%">
        ...
        <?php   $dataquery = $DB->get_records('searchda_categories');
        $i=1;
        ...
        ?>
             <tr class="<?php if($value->status==1){ echo 'myststus'; } ?>">
                <td><?=  $i++;  ?></td>
                     <td><a href="<?php echo $CFG->wwwroot ?>/blocks/searchdashboard/second_categories.php?id=<?= $value->id; ?>">   <?= $value->title ?> </a>  </td>
                <td><?php if($value->iconfiletype=='fontawesome'){
                  echo '<i class="fa '.$value->classname.'" aria-hidden="true"></i>';
                }
                  ...
                  if($value->iconfiletype=='svg'){
                    $fileinfo = $DB->get_record_sql("SELECT * FROM {files} WHERE itemid=$value->icon_file and filename!='.'");
                      ...
                      echo '<img src="'.$backgrounddata.'" style="height: 16px;">';
                    }
                }
                ...
                <a href="<?php echo $CFG->wwwroot ?>/blocks/searchdashboard/first_categories_edit.php?id=<?= $value->id; ?>"><span class="pencil"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></span></a>
                    <a onclick="return confirm('Are you sure you want to status update -<?= $value->title ?>')" href="<?php echo $CFG->wwwroot ?>/blocks/searchdashboard/status_update.php?id=<?= $value->id; ?>&table=searchda_categories&url=<?php echo $CFG->wwwroot ?>/blocks/searchdashboard/first_categories.php"> <span class="eyes"><i class="fa <?php if($value->status==1){ echo 'fa-eye-slash'; }else{ echo'fa-eye'; } ?>" aria-hidden="true"></i></span></a>
                  <a href="<?php echo $CFG->wwwroot ?>/blocks/searchdashboard/delete.php?id=<?= $value->id; ?>&table=searchda_categories&url=<?php echo $CFG->wwwroot ?>/blocks/searchdashboard/first_categories.php" onclick="return confirm('Are you sure you want to delete <?= $value->title ?> Categories')">  <span class="del"><i class="fa fa-trash" aria-hidden="true"></i></span> </a>
                  </div>
                </td>
