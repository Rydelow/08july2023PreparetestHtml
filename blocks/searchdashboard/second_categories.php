...
$PAGE->navbar->add(($topbardata->title), new moodle_url('/blocks/searchdashboard/second_categories.php?id='.$id));
...
<div class="row">
  <div class="col-md-10">
    <table id="example" class="display" cellspacing="0" width="100%">
        ...
        <?php   $dataquery = $DB->get_records('searchda_secondc',array('categoriesid'=>$id));
        $i=1;
        ...
        ?>
             <tr class="<?php if($value->status==1){ echo 'myststus'; } ?>">
                <td><?=  $i++;  ?></td>
                     <td><a href="<?php echo $CFG->wwwroot ?>/blocks/searchdashboard/third_categories.php?subid=<?= $value->id; ?>">   <?= $value->title ?> </a>  </td>
               
                     
          
                <td>
                  <div class="ic_o_n text-center">
                <a href="<?php echo $CFG->wwwroot ?>/blocks/searchdashboard/second_categories_edit.php?cid=<?= $value->id; ?>&id=<?= $id; ?>"><span class="pencil"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></span></a>
                    <a onclick="return confirm('Are you sure you want to status update -<?= $value->title ?>')" href="<?php echo $CFG->wwwroot ?>/blocks/searchdashboard/status_update.php?id=<?= $value->id; ?>&table=searchda_secondc&url=<?php echo $CFG->wwwroot ?>/blocks/searchdashboard/second_categories.php?id=<?= $id; ?>"> <span class="eyes"><i class="fa <?php if($value->status==1){ echo 'fa-eye-slash'; }else{ echo'fa-eye'; } ?>" aria-hidden="true"></i></span></a>

                  <a href="<?php echo $CFG->wwwroot ?>/blocks/searchdashboard/delete.php?id=<?= $value->id; ?>&table=searchda_secondc&url=<?php echo $CFG->wwwroot ?>/blocks/searchdashboard/second_categories.php?id=<?= $id; ?>" onclick="return confirm('Are you sure you want to delete <?= $value->title ?> Categories')">  <span class="del"><i class="fa fa-trash" aria-hidden="true"></i></span> </a>
                  </div>
                </td>
