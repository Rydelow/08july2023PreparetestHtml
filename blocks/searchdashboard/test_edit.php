...
if(!empty($_GET['id'])){
$PAGE->navbar->add(($topbardata->title), new moodle_url('/blocks/searchdashboard/test_edit.php?id='.$id));
$PAGE->set_url('/blocks/searchdashboard/test_edit.php?id='.$id);
}
...
$topbardataa = $DB->get_record('searchda_test',array('id'=>$cid));
...
if ($mform->is_cancelled()) {
 ...
 $topbar_slider->title=$fromform->title;
  ...
  $DB->insert_record('searchda_test', $topbar_slider,true);
redirect($CFG->wwwroot."/blocks/searchdashboard/exam_explore.php?sucessfully=inserted&id=".$fromform->thid);
  ...
  $DB->update_record('searchda_test', $topbar_slider,true);
